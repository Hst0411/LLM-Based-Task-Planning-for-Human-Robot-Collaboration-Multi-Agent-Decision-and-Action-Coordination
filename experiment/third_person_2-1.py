# robot 裝上第一人稱 camera (perception module 辨識 human)
# 計算 robot 花費的 frames
# 協作策略：human 挑 6 筆
# robots 拍到 human 拿著食物則標記
...
import random
import os, glob
from experiment.gpt_check_class import detect_and_check
...
def mark_task_finished(food_id: Union[int, str], task_list_path="experiment/robot_task_list.json"):
...
def senario():
    ...
    human_position = {"x": -9.0, "y": 0, "z": 2.6}
    c.start_floorplan_trial(scene="5a", layout="0_0", replicants=[replicant_position, human_position],
    ...
    # robot 看到 human 拿 apple
    # robot_tasks = LLM_robot_tasks(c)
    ...
    while any(active):
        ...
        if active[0]:
            ...
            # 每 50 幀觸發一次檢查
            if frame_count % 50 == 0:
                last_idx, events = detect_and_check(start_index=last_idx)
                for e in events:
                    print("🔔 Event:", e)
                    ...
                    if "detected a human carrying" in e:
                        ...
                        print(f"🍎 Robot detected human carrying food: {food_name}")
                        mark_task_finished(food_id=food_name)
                        # indices[0] += 4
...
if __name__ == "__main__":
    ...
    shutil.copyfile('experiment/task_list.json', 'experiment/robot2_task_list.json')
    shutil.copyfile('experiment/task_list.json', 'experiment/human_task_list.json')
    senario()
